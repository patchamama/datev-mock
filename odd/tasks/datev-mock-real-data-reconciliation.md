...
- [x] W4 — RED+GREEN batch C: expand `addressees` (historical-array
...
- [x] W5 — Full regression + README/task-doc updates reflecting the
...
- [x] W6 — Commit + push (per-batch throughout, plus this final W5 wrap-up
      commit).
...
- 2026-09-23: W4 done (RED+GREEN, delegated direct). Field expansion for `addressees`, XML negotiation for `posting_proposal_rules_incoming`/`outgoing` and `terms_of_payment`, new JSON projection for `master_data.clients`. 274/274 tests passing (257 + 17 new). Route: delegated direct (touched `app/models.py`, `app/fake_data.py`, `app/json_serializers.py`, `app/xml_serializers.py`, `app/routers/accounting.py`, `app/routers/master_data.py`, 4 test files — well over the Writer trigger).
  - **Correction to this doc's own field table (2nd instance, same class of error W3 already found once)**: the `master_data.clients` JSON section above claimed "exactly these 7 fields" (`id`, `legal_person_id`, `name`, `number`, `status`, `timestamp`, `type`), derived from a first-record summary. Parsing the full 100-record `examples/master-data-clients.xml` directly found 2 more real, genuinely optional fields the doc missed: `client_since` (14/100) and `client_to` (1/100), both mapping to `ClientResource.ClientSince`/`.ClientTo` (already modeled for XML). Included in the projection. `number`'s int type re-verified directly (unquoted on all 100 records) — correct, not a repeat of W1's string bug.
  - **Correction to this doc's own claim about `addressees`**: the doc said the model's fingerprint "uses a flat `surnames` field" needing replacement — grepped `app/models.py`/`app/fake_data.py`/`app/overrides.py` before editing and found zero occurrences of `surnames` anywhere; there was only `current_surname` (scalar). No flat field to remove, just new fields to add — simpler than the doc implied.
  - **`addressees`**: new `HistoricalValue` dataclass (`value: str`, `valid_from: Optional[str] = None`); `Addressee` gained `company_names`/`short_names`/`legal_form_ids`/`surnames` as `Optional[list[HistoricalValue]]`. `sex`/`date_of_foundation` already existed pre-W4 (verified, not re-added). `_generate_addressees` populates real sparsity rates (`surnames` 8/8 on natural_person, `company_names` 103/103 on legal_person, `legal_form_ids` ~73% of legal_person, `short_names` ~83% both types). Verified natural/legal-person field exclusivity and the `{value[, valid_from]}` shape by parsing the full 111-record file, not sampling — doc's field list (19 top-level fields) held up here. `master_data.py`'s `_strip_none` made recursive so nested `valid_from=None` is omitted, not `null`.
  - **`posting_proposal_rules_incoming`/`outgoing`**: no field changes (confirmed — both real captures are empty `[]`, zero shape evidence). XML negotiation added, **inferred by pattern only** (`ArrayOfPostingProposalRule`/`...Contracts.PostingProposalRule`); nested/list fields (`assignment_criteria`, `posting_proposal_information`) excluded from XML, same precedent as W2's `CostCenter.cost_rates`/`.properties`.
  - **`terms_of_payment`**: doc's "already matches exactly" claim verified correct by direct read of `examples/terms-of-payment.xml` — no model change. XML negotiation added, **inferred by pattern** (real capture is JSON content despite the `.xml` filename, same quirk as `fiscal_years`).
  - **`app/overrides.py` fingerprints**: verified all 4 relevant fingerprints (`master_data.addressees`, `accounting.posting_proposal_rules_incoming`/`outgoing`, `accounting.terms_of_payment`, `master_data.clients` XML root map) — no conflicts, no changes needed.
  - **Native review (RDD)**: user had RDD enabled; medium-risk candidate (10 files, 672 lines) reviewed via `review-reliability` lens, **approved** with 4 non-blocking advisory findings (test-coverage gaps, not defects): `legal_form_ids`' `valid_from` is unconditionally absent in fake data despite the docstring claiming a real ~7% presence rate (a real inconsistency between docs and generator — worth a small follow-up fix, not done in W4); no test asserts `valid_from` is ever actually populated for any historical-array field; `master_data.clients`' new `_negotiate_format` only has its JSON and default/XML-fallback branches tested, not an explicit `Accept: application/xml`; no test sends a genuinely ambiguous (multi-format/wildcard) `Accept` header for the 3 newly-negotiated endpoints. Acknowledged, authority burned — logged here as known follow-up, not fixed in this batch (non-blocking per the review's own disposition).
  - Self-check: parsed the full `examples/addressees.xml`/`master-data-clients.xml`/`terms-of-payment.xml` directly, extracted every real value read, grepped all 10 touched files against that value list (553 unique values) — zero real-value matches (only coincidental short-substring collisions with pre-existing fictitious pool entries/doc-comment numbers).
  - Live check (port 58471): curled `master-data/v1/addressees` JSON, `posting-proposal-rules-incoming-invoices` XML+JSON, `terms-of-payment` XML+JSON, `master-data/v1/clients` JSON projection + default XML — all correct shapes/roots. Stopped via `taskkill //F //PID 23844`; verified port 58471 free afterward. `.gitignore` confirmed untouched.
  - Final: `274 passed, 2 warnings in 3.72s` (pytest tests/ -q).
- 2026-09-23: W5 done (full regression + README/task-doc updates). Full `pytest tests/ -v` regression confirmed 274/274 before starting doc work.
  - **Gap found during W5's own live catalog smoke test**: `accounting.assets_stocktakings` — one of the 15 accounting sub-resources the user's "todo: campos completos + XML en los 15" scope decision covers — was never included in W1-W4's batching (an orchestrator planning miss, not a subagent error; it's absent from every one of W2/W3/W4's task briefs). `examples/info.txt` lists a stocktaking capture attempt (item 15) but no `stocktakings.xml` file was ever actually saved to `examples/` — confirmed by directory listing, so there is genuinely zero real evidence for this endpoint, the same situation `cost_centers`/`posting_proposal_rules_incoming/outgoing` were already in. Fixed in W5 directly (small, single-endpoint, fully mechanical — same established pattern as 4 prior batches, done inline rather than delegated): added XML negotiation only (no field changes — no evidence to expand from), `ASSET_STOCKTAKING_FIELD_ORDER` (`general_ledger_account`, nested, excluded — same precedent as `CostCenter.cost_rates`/`.properties`), `ASSET_STOCKTAKING_NS` (inferred by pattern), `serialize_assets_stocktakings`, wired `get_assets_stocktakings` to the shared `_negotiate_format`. Fixed the one bare (no-`Accept`-header) GET test that broke once this endpoint started defaulting to XML (same fix pattern as every prior batch), added an XML root/namespace test and a default-format test. `app/overrides.py`'s existing fingerprint (`{asset_number, inventory_number}`) needed no change (no field changes made). 276/276 tests passing (274 + 2 new). Verified live (port 58473): XML root `ArrayOfAssetStocktaking`, JSON shape unchanged, default format XML.
  - **README.md** rewritten to reflect the corrected understanding: content negotiation (`Accept`-header based XML/JSON) is now documented as the universal mechanism across all 15 accounting sub-resources (previously documented as `accounting/v1/clients`-specific only, with the other extended endpoints described as "JSON-only — no real XML evidence exists for any of these", which stopped being true as of W2). New "Content negotiation" key-decisions subsection explains confirmed-vs-inferred-by-pattern XML evidence per endpoint and links to this epic doc. Test badge/count updated 212 → 276. Status section lists this 6th epic. Endpoints table split to show which endpoints are XML+JSON vs JSON-only vs XML-only. Noted quirk corrections (accounting.clients `number` as int, `main_function`/`main_function_number` include `0`, `has_dunning_block` not `dunning_level`, no invented nested person sub-objects by default) with a pointer to this epic doc's W1 log for the full evidence trail, rather than duplicating it.
  - **`odd/tasks/datev-mock-extended-endpoints.md`**: added a pointer note near its "all new endpoints are JSON-only" claim (kept as historical record, not rewritten) noting that claim was superseded for the 15 accounting sub-resources by this epic (W2-W5) — Master Data endpoints (`addressees`, `banks`) remain JSON-only, unaffected.
  - No new real-value leak risk in this batch — no new real capture files were read (the gap fix used zero real evidence, by definition; doc edits only referenced already-established facts from this epic's own earlier entries).
